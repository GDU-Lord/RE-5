...
### [Шейдеры](#шейдеры)
- [new RectJS.Shader()](#soundunbindobject)
- [new RectJS.Program()](#soundunbindobject)
### [Параметры вершинного шейдера](#параметры-вершинного-шейдера)
...
### [Параметры фрагментного шейдера](#параметры-фрагментного-шейдера)
- [FS uniform tex](#uniform-sampler2d-tex)
- [FS varying vColor](#varying-vec4-vcolor-1)
- [FS varying vUV](#varying-vec4-vuv-1)
...
- __options__ `<object>`
	...
	- __colorMode__ `<string>` __*Default:*__ `SINGLE` - режим отрисовки цвета. Есть три режима:
		...
		- `"VERTEX"` - работает как "SINGLE", но озволяет задать цвет каждой вершине (увета задаются в свойстве __colors__)
